...
@MainActor @Observable final class ConversationCoordinator {
    ...
    func start() {
        ...
        connectionTask = Task { [weak self] in
            guard let self else { return }
            do { try await self.transport.connect(api: self.api, instructions: instructions, history: history, languageCode: self.language.id, speechRate: self.store.preferences.speechRate) }
    ...
    @discardableResult func sendTyped(_ text: String) async -> Bool {
        ...
        do {
            ...
            transport.speak(result.text, languageCode: language.id, rate: store.preferences.speechRate)
            activity.learnerEngaged(now: activityNow); inactivitySeconds = nil
            #if DEBUG && targetEnvironment(simulator)
