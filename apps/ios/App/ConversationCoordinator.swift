...
@MainActor @Observable final class ConversationCoordinator {
    ...
    init(store: LearningStore) {
        ...
        meanings = MeaningController { request in
            ...
            let result = try await api.respond(instructions: TeachingPolicy.translation(language: language, meaningLanguage: request.meaningLanguage), input: request.translationInput, customModel: store.preferences.customModel)
    ...
    private static func assess(api: APIClient, snapshot: SessionRecord, passage: Passage, customModel: String = "") async throws -> FinalAssessmentResult {
        guard let language = LanguageRegistry.module(for: snapshot.languageID) else { throw ArchiveError.unsupportedLanguage }
        let result = try await api.respond(instructions: TeachingPolicy.assessment(language: language), input: TeachingPolicy.context(snapshot, passage: passage), schema: APIClient.assessmentSchema(language: language), customModel: customModel)
    ...
    private func scheduleAssessment() {
        ...
        assessmentTask = Task { [weak self] in
            do {
                ...
                let result = try await Self.assess(api: self.api, snapshot: snapshot, passage: p, customModel: self.store.preferences.customModel)
    ...
    private func typedReplyResponse(instructions: String, input: String) async throws -> APIResult {
        ...
        #endif
        return try await api.respond(instructions: instructions, input: input, customModel: store.preferences.customModel)
    }
    ...
    func currentTopic(_ query: String) async throws -> TopicBrief {
        ...
        guard hasAIConsent else { throw AIProcessingConsent.ConsentError.required }
        let result = try await api.respond(instructions: TeachingPolicy.currentTopic(language: targetLanguage), input: String(query.prefix(500)), search: true, customModel: store.preferences.customModel)
        guard generation == languageGeneration else { throw CancellationError() }
